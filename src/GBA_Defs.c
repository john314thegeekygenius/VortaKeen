...
GBA_ARM void GBA_VSyncIRQ() {
	...
	*(volatile uint16_t*)GBA_INT_ENABLE = 0;
...
#ifdef GBA_CUSTOM_VSYNC_INT_FUN
		GBA_VSyncFunction();
#endif
    ...
    //if ((*(volatile uint16_t*)GBA_INT_STATE & GBA_INT_VBLANK) == GBA_INT_VBLANK) {
		...
		#ifndef GBA_CUSTOM_VSYNC_INT_FUN
...
//		#ifdef GBA_MIX_MY_AUDIO
    ...
    *(volatile uint16_t*)GBA_INT_ACK = 0x1;
...
void GBA_SetGlobalVolume(short L_Volume, short R_Volume){
	...
	*(volatile uint16_t*)GBA_SOUNDCNT_L |= L_Volume | (R_Volume<<4);
	...
	// Set the GB channels to full volume
...
void GBA_PlayGBChannel(short id){

	*(volatile uint16_t*)GBA_SOUNDCNT_L = 0;
	...
	switch(id){
		case 1:
			...
			*(volatile uint16_t*)GBA_SOUNDCNT_L |= GBA_SND_1L | GBA_SND_1R ;
			break;
		case 2:
			//enable sound 2 to left and right
			*(volatile uint16_t*)GBA_SOUNDCNT_L |= GBA_SND_2L | GBA_SND_2R ;
			break;
		case 3:
			//enable sound 3 to left and right
			*(volatile uint16_t*)GBA_SOUNDCNT_L |= GBA_SND_3L | GBA_SND_3R ;
			break;
		case 4:
			//enable sound 4 to left and right
			*(volatile uint16_t*)GBA_SOUNDCNT_L |= GBA_SND_4L | GBA_SND_4R ;
			break;
	}
	//full volume, left and right
	*(volatile uint16_t*)GBA_SOUNDCNT_L |= 0x77;

};

void GBA_StopGBChannel(short id){
	...
	switch(id){
		case 1:
			...
			*(volatile uint16_t*)GBA_SOUNDCNT_L &= ~(GBA_SND_1L | GBA_SND_1R);
			break;
		case 2:
			//disable sound 2 to left and right
			*(volatile uint16_t*)GBA_SOUNDCNT_L &= ~(GBA_SND_2L | GBA_SND_2R);
			break;
		case 3:
			//disable sound 3 to left and right
			*(volatile uint16_t*)GBA_SOUNDCNT_L &= ~(GBA_SND_3L | GBA_SND_3R);
			break;
		case 4:
			//disable sound 4 to left and right
			*(volatile uint16_t*)GBA_SOUNDCNT_L &= ~(GBA_SND_4L | GBA_SND_4R);
			break;
	}
	//full volume, left and right
...
void GBA_StopGBSounds(){
	//no volume, left and right
	*(volatile uint16_t*)GBA_SOUNDCNT_L = 0x00;
};
